...
This custom build branch updated to build using android-ndk 16b (newer android-ndk versions do not support the old armeabi target).

To complete the =make . The =ANDROID_NDK_ROOT= environment variable must be defined which should point to your NDK root. Once you have cloned the repo, change directory into the root of the repository and run the following commands:
